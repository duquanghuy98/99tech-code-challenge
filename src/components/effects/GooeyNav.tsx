/**
 * GooeyNav - official React Bits component (reactbits.dev/components/gooey-nav)
 * Minimal change: href/label items replaced with react-router to/labelKey.
 */
...
const GooeyNav: React.FC<GooeyNavProps> = ({
  ...
  const getActiveIndex = () =>
    items.findIndex((item) => location.pathname === item.to);

  const [activeIndex, setActiveIndex] = useState<number>(() => Math.max(0, getActiveIndex()));
  ...
  return (
    <>
      {/* Verbatim style block from official React Bits source */}
      <style>{`
        :root {
          --color-1: #ffffff;
          --color-2: #ffffff;
      ...
      <div className="gooey-wrap relative" ref={containerRef} style={{ transform: 'translate3d(0,0,0.01px)' }}>
            ...
            {items.map((item, index) => (
              ...
              >
                <a
                  ...
                  className="outline-none py-[0.6em] px-[1em] inline-block text-sm font-medium"
                >
                  {t(item.labelKey)}
                </a>
              </li>
